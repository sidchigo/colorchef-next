...
export function findColors(inputColor, colorList = [], scale, ts, limit = 120) {
    ...
    let sampleColors =
        colorList.length !== 0
            ? colorList
            : generateNColors(1000, originalColor + ts);
    let foundColors = [],
        pair,
        contrastResult,
        result,
...
export function generateDarkPalette(primary, secondary, accent) {
    let tempPrimary, darkPrimary, darkSecondary, darkAccent;
    tempPrimary = tinycolor(primary).toHsl();
    console.log(tinycolor(primary).toHsl());
    // tempPrimary.s = 0.2;
    tempPrimary.l = 0.075;
...
export function paletteGenerator(limit = 30) {
    ...
    for (let i = 0; i < limit; i++) {
        ...
        if (
            ...
            accentColor =
                accentArray.colors[
                    Math.floor(Math.random() * accentArray.colors.length)
                ];
